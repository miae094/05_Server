 ...
 * 	<servlet> <server-mapping> 을 작성하는 효과
 * 
 * */
...
public class ExampleController2 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// -> JSP 가 대신 화면을 만들라고 요청(Servlet 이 JSP에게 요청/응답을 위임)
		
		// JSP 가 대신 응답 화면을 만들어 주기 위해서는
		...
		// webapp : 브라우저 통해서 접근 가능
		// WEB-INF : 서블릿을 통해 접근 가능
		
		// WEB-INF 폴더에 있는 파일들은 클라이언트가 직접적으로 접근할 수 없는 영역,
		// 서블릿을 통해서만 접근 가능
		
		dispatcher.forward(req, resp);
		// RequestDispatcher 객체를 사용하여 현재 요청(req)와 응답(resp)를 지정한
		// JSP 페이지(result.jsp)로 전달하는 작업
		// -> 즉, 현재 서블릿(ExampleController2)에서 처리하던 요청을 result.jsp 로 전달하고
		// 		제어권을 그곳으로 넘기는 것(== 위임)
