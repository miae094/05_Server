...
<body>
	...
	<c:remove var="test2" scope="request"/>
	...
	<c:if test='${test3 == "금요일"}'>		<%-- 문자열 비교시 홑따옴표를 밖에 쓰고 안에 쌍또옴표를 써야함 --%>
		<h4>${test3 } 짱좋다!!</h4>
	</c:if> 
	<c:if test='${test3 != "금요일"}'>
		<h4>너무싫다,,,</h4>
	...
	<c:choose>
		<%--
			...
			1) null 인 경우 true
			2) 연산 대상이 배열이나 컬렉션인데 내부에 아무 요소도 없는 경우
		...
		<c:otherwise><h4>성인입니다</h4></c:otherwise>
	</c:choose>
