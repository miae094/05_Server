...
public class ELTestServlet2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 2. requtest scope
		// - 요청 받은 Servlet 과 요청이 위임된 JSP 에서 유지되는 객체
		
		// 1) 객체에 값(속성) 추가하는 방법
		// 		범위객체.setAttrivute("key", value);
		
		// 2) 객체에 값(속성) 얻어오는 방법
		// 		범위객체.getAttribute("key");
		// -> 반환형 Object -> 필요 시 다운캐스팅
		req.setAttribute("requestValue", "request scope 객체에 세팅한 값");
		// requestAvlue=request scope 객체에 세팅한 값
		...
		// 4. application scope
		// - 서버 전체에 단 한개만 존재하는 객체(서버 켜지자마자 생성되서 한개만 유지되다가 서버 종료되면 소멸)
		// -> 모든 클라이언트가 공유
		// -> 서버 시작 시 생성, 서버 종료 시 소멸
		
		// 1) application scope 객체 얻어오기
		ServletContext application = req.getServletContext();
		// ServletContext application = session.getServletContext(); 도 가능. 자기보다 작은 범위에서 가져오면 된다.
